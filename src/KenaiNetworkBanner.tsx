...
export function KenaiNetworkBanner() {
  return (
    <div className="border-b border-white/10 bg-[var(--color-surface-elevated)]/80 backdrop-blur">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        ...
        <div className="flex flex-wrap gap-2">
          {networkLinks.map((site) => (
            <a key={site.href} href={site.href} target="_blank" rel="noreferrer" className="rounded-full border border-white/10 bg-[var(--color-surface)] px-3 py-1 text-xs font-semibold text-[var(--color-text)] transition hover:border-[var(--color-primary)] hover:text-[var(--color-primary)]">
              {site.label}
            </a>
          ))}
        </div>
