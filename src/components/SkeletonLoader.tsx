...
export default function SkeletonLoader({ variant = 'card', count = 3, className = '' }: SkeletonLoaderProps) {
  if (variant === 'detail') {
    return (
      <div className={`grid gap-6 lg:grid-cols-[1.1fr_.9fr] ${className}`}>
        <div className="space-y-4 rounded-[32px] border border-white/10 bg-[var(--color-surface)]/90 p-6">
    ...
    return (
      <div className={`grid gap-4 ${className}`}>
        {Array.from({ length: count }).map((_, index) => (
          <div key={index} className="flex gap-4 rounded-[28px] border border-white/10 bg-[var(--color-surface)]/90 p-4">
  ...
  return (
    <div className={`grid gap-6 md:grid-cols-2 xl:grid-cols-3 ${className}`}>
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="overflow-hidden rounded-[28px] border border-white/10 bg-[var(--color-surface)]/90">
